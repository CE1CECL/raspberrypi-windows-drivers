...
--*/
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology)
    {
        ...
        // Install RPIHDMI0WAV topology miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with RPIHDMI0WAV.<TopoName>.szPname in the inf's [Strings] section
                                    CLSID_PortTopology,
                                    CLSID_PortTopology, 
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave)
    {
        ...
        // Install RPIHDMI0WAV wave miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with RPIHDMI0WAV.<WaveName>.szPname in the inf's [Strings] section
                                    CLSID_PortWaveRT,
                                    CLSID_PortWaveRT,   
