...
typedef struct _ENDPOINT_MINIPAIR 
{
    ...
    // Topology miniport.
    PWSTR                           TopoName;               // make sure this name matches with RPI4HDMIWAV.<TopoName>.szPname in the inf's [Strings] section
    PFNCREATEMINIPORT               TopoCreateCallback;
    PCFILTER_DESCRIPTOR*            TopoDescriptor;

    // Wave RT miniport.
    PWSTR                           WaveName;               // make sure this name matches with RPI4HDMIWAV.<WaveName>.szPname in the inf's [Strings] section
