...
--*/
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology)
    {
        ...
        // Install RPI4HDMIWAV topology miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with RPI4HDMIWAV.<TopoName>.szPname in the inf's [Strings] section
                                    CLSID_PortTopology,
                                    CLSID_PortTopology, 
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave)
    {
        ...
        // Install RPI4HDMIWAV wave miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with RPI4HDMIWAV.<WaveName>.szPname in the inf's [Strings] section
                                    CLSID_PortWaveRT,
                                    CLSID_PortWaveRT,   
